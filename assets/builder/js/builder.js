...
editableItems['.fa'] = ['color', 'font-size', 'opacity'];
...
editableItems['p, li'] = ['color', 'font-size', 'background-color', 'font-family'];
editableItems['a, a.btn, a.download-btn, button.btn, a.goto'] = ['color', 'border-color', 'border-width', 'border-radius', 'font-size', 'background-color'];
editableItems['input'] = ['color', 'background-color'];
...
editableItems['section, div'] = ['background-image', 'background-color', 'background-repeat', 'background-size', 'background-attachment', 'padding-top', 'padding-bottom', 'border-bottom-width', 'border-bottom-color'];
editableItems['footer'] = ['background-image', 'background-color', 'background-repeat', 'background-size', 'border-bottom-width', 'border-bottom-color', 'border-top-width', 'border-top-color'];
...
editableItemOptions['div : background-repeat'] = ['no-repeat', 'repeat', 'repeat-x', 'repeat-y'];
editableItemOptions['div : background-size'] = ['cover', 'auto'];
editableItemOptions['div : background-attachment'] = ['fixed', 'scroll'];
...
var editableContent = ['.editContent', 'input', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', '.navbar a', 'button', 'a.btn', 'a.download-btn', '.footer a', '.tableWrapper', '.filter a', '.quote small', '.quote span', '.pricing-table span', '.benefits-list', '.data-list', '.links-list', '.post-desc', '.post-info', '.widget li a', '.tags', '.panel-body', 'td', 'th', '.num-icon', 'li'];
...
function setPendingChanges(v) {
	...
	if( v == true ) {
		...
		$('#savePage .bLabel').text("Simpan");
		
		pendingChanges = true;
	
	} else {
	
		$('#savePage .bLabel').text("Tidak ada perubahan!");
...
$(function(){
					...
					$(this).contents().find( pageContainer + ' '+editableContent[i] ).hover( function(){
						...
						if( this.tagName == 'SMALL' || this.tagName == 'A' || this.tagName == 'LI' || this.tagName == 'SPAN' || this.tagName == 'B' || this.tagName == 'I' || this.tagName == 'TT' || this.tageName == 'CODE' || this.tagName == 'EM' || this.tagName == 'STRONG' || this.tagName == 'SUB' || this.tagName == 'BUTTON' || this.tagName == 'LABEL' || this.tagName == 'P' || this.tagName == 'H1' || this.tagName == 'H2' || this.tagName == 'H2' || this.tagName == 'H3' || this.tagName == 'H4' || this.tagName == 'H5' || this.tagName == 'H6' ) {
							...
							$('#editContentModal #contentToEdit').redactor({
								buttons: ['html', 'formatting', 'alignment', 'bold', 'italic', 'deleted', 'link'],
								focus: true,
	...
	$('#exportModal > form').submit(function(){
		//action after export/saving
	
		$('#exportModal > form #exportSubmit').hide('');
		
		$('#exportModal > form #exportCancel').text('Go To Client Area');

		$('#exportModal > form #exportCancel').attr('onClick','javascript:location.href="http://localhost/smartmedia/member/"');

		$('#exportModal > form #exportCancel').removeAttr('data-dismiss');
